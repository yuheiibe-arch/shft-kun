...
function renderShiftBlock(ss, originalLocName, finalSheetName, yearMonthStr, monthData, targetTerm) {
  ...
  let isNewlyCreatedSheet = false;
  let originalMaxRows = 0;

  if (!sheet) {
    sheet = safeExecute(() => ss.insertSheet(finalSheetName, ss.getNumSheets()), 3, "新規シート作成");
    isNewlyCreatedSheet = true;
  } else {
    originalMaxRows = sheet.getMaxRows();
  }
  ...
  try {
    ...
    if (lastRow > 0) {
      let searchData = sheet.getRange(1, 1, lastRow, 4).getValues();
      for (let i = 0; i < searchData.length; i++) {
        if (searchData[i][0] === "適用開始") {
          let cellVal = searchData[i][3];
          let dStr = "";
          if (cellVal instanceof Date) {
            dStr = Utilities.formatDate(cellVal, Session.getScriptTimeZone(), "yyyy/MM/dd");
          } else {
            dStr = String(cellVal);
          }
          
          if (dStr.startsWith(yearMonthStr)) {
            startRow = (i + 1) - 15; 
    ...
    // =========================================================
    // ★ 諸悪の根源（ループ内の通信）を撤廃するための配列群
    // =========================================================
    let dateValues = []; // A列・B列（曜日・週番号）を一括書込するための配列
    let writeValues = [];
    let writeBgs = [];
    let writeFonts = [];
    let writeRules = [];
    ...
    // =========================================================
    // ★ ループを抜けたあと、配列に貯めたデータを「1回の通信」で一撃書込！
    // =========================================================
    const dateRange = sheet.getRange(startRow + 19, 1, dateValues.length, 2); // A・B列用
    const targetRange = sheet.getRange(startRow + 19, 4, writeValues.length, 12); // D列以降用

    let shouldWrite = true;
    if (!isNewBlock) {
      let currentValues = safeExecute(() => targetRange.getValues(), 3, "既存データの取得");
    ...
    safeExecute(() => {
      // 日付(A・B列)の一括書き込み（※これで通信124回が「1回」に削減されました）
      dateRange.setValues(dateValues).setHorizontalAlignment("left");
      ...
      SpreadsheetApp.flush(); // ★ 月ごとの確実なメモリ解放
      
    }, 3, "シートへの一括描画");

  } catch (e) {
    console.error(`[🚨 ロールバック発動] ${finalSheetName} の書き込み中にエラーが発生しました。ゴミデータを削除します。詳細: ${e.message}`);
    
    if (isNewlyCreatedSheet) {
      safeExecute(() => ss.deleteSheet(sheet), 3, "失敗した新規シートの削除");
    } else {
      let currentMax = sheet.getMaxRows();
      if (currentMax > originalMaxRows) {
        safeExecute(() => sheet.deleteRows(originalMaxRows + 1, currentMax - originalMaxRows), 3, "失敗した追記行の削除");
      }
    }
    throw e; 
  }
